...
CONFIG_PATH = os.path.join(BASE_DIR, 'autorole.json')
ROOT_CONFIG_PATH = 'autorole.json'
...
class AutoRoleCog(commands.Cog):
    ...
    def load_config(self):
        # Load from local folder, fallback to root, then save locally
    ...
    @app_commands.command(name='reloadautorole', description='Ricarica la configurazione di AutoRole (solo admin)')
    async def reload_autorole(self, interaction: discord.Interaction):
        if not interaction.user.guild_permissions.administrator and not is_owner(interaction.user):
            await interaction.response.send_message('❌ Non hai i permessi per usare questo comando!', ephemeral=True)
            return
        try:
            self.load_config()
            await interaction.response.send_message('✅ Configurazione AutoRole ricaricata con successo!', ephemeral=True)
            logger.info(f'Configurazione AutoRole ricaricata da {interaction.user.name}#{interaction.user.discriminator} ({interaction.user.id}) in {interaction.guild.name}')
        except Exception as e:
            await interaction.response.send_message(f'❌ Errore nel ricaricare la configurazione AutoRole: {e}', ephemeral=True)
            logger.error(f"Errore reloadautorole da {interaction.user.name}#{interaction.user.discriminator} ({interaction.user.id}) in {interaction.guild.name}: {e}")
