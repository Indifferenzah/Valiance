...
TICKETMSG_JSON = os.path.join(BASE_DIR, 'ticketmsg.json')
TICKET_JSON = os.path.join(BASE_DIR, 'ticket.json')
CLOSED_TICKETS_JSON = os.path.join(BASE_DIR, 'closed_tickets.json')
...
class TicketCog(commands.Cog):
    def __init__(self, bot):
        ...
        if os.path.exists(TICKETMSG_JSON):
            with open(TICKETMSG_JSON, 'r', encoding='utf-8') as f:
                try:
                    self.ticket_messages = json.load(f)
                except Exception:
                    self.ticket_messages = {}
        elif os.path.exists('ticketmsg.json'):
            with open('ticketmsg.json', 'r', encoding='utf-8') as f:
                try:
                    ...
                    self.ticket_messages = {}
            try:
                with open(TICKETMSG_JSON, 'w', encoding='utf-8') as f:
                    json.dump(self.ticket_messages, f, indent=2, ensure_ascii=False)
            except Exception:
                pass
        self.ticket_owners = {}
        ...
        elif os.path.exists('ticket.json'):
            with open('ticket.json', 'r', encoding='utf-8') as f:
                loaded = json.load(f)
                self.ticket_owners = {int(k): v for k, v in loaded.items()}
            try:
                with open(TICKET_JSON, 'w', encoding='utf-8') as f:
                    json.dump(self.ticket_owners, f, indent=2, ensure_ascii=False)
            except Exception:
                pass
        self.closed_tickets = {}
        if os.path.exists(CLOSED_TICKETS_JSON):
            with open(CLOSED_TICKETS_JSON, 'r', encoding='utf-8') as f:
                try:
                    self.closed_tickets = json.load(f)
                except Exception:
                    self.closed_tickets = {}
        elif os.path.exists('closed_tickets.json'):
            with open('closed_tickets.json', 'r', encoding='utf-8') as f:
                try:
                    self.closed_tickets = json.load(f)
                except Exception:
                    self.closed_tickets = {}
            try:
                with open(CLOSED_TICKETS_JSON, 'w', encoding='utf-8') as f:
                    json.dump(self.closed_tickets, f, indent=2, ensure_ascii=False)
            except Exception:
                pass
        self.blacklist = []
        ...
        elif os.path.exists('blacklist.json'):
    ...
    async def slash_rename_ticket(self, interaction: discord.Interaction, new_name: str):
        ...
        try:
            ...
            if log_cog:
                ticket_number = self.ticket_owners.get(channel.id, {}).get('number', 'N/A')
                await log_cog.log_ticket_rename(channel.mention, new_name, str(ticket_number), interaction.user.mention)
    ...
    @app_commands.command(name='sendtranscript', description='Manda via DM il transcript di un ticket chiuso a un utente')
    @app_commands.describe(number='Numero del ticket', user='Utente a cui inviare il transcript')
    async def slash_sendtranscript(self, interaction: discord.Interaction, number: int, user: discord.User):
        staff_role_id = self.config.get('ticket_staff_role_id')
        has_staff = False
        try:
            if hasattr(interaction.user, 'roles') and staff_role_id:
                has_staff = any(role.id == int(staff_role_id) for role in getattr(interaction.user, 'roles', []))
        except Exception:
            has_staff = False
        if not (has_staff or interaction.user.guild_permissions.administrator or interaction.user.id == OWNER_ID):
            await interaction.response.send_message('❌ Non hai i permessi per usare questo comando!', ephemeral=True)
            return

        ticket_str = str(number)
        filename = None
        if ticket_str in self.closed_tickets:
            filename = self.closed_tickets[ticket_str].get('transcript_file')
        if not filename:
            filename = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'transcripts', f'transcript-{ticket_str}.txt')
        ...
        try:
            ...
            await dm.send(content=f'Transcript del ticket #{ticket_str}', file=discord.File(fp=filename))
            if interaction.response.is_done():
                await interaction.followup.send(f'✅ Transcript del ticket #{ticket_str} inviato in DM a {user.mention}.', ephemeral=True)
            else:
                await interaction.response.send_message(f'✅ Transcript del ticket #{ticket_str} inviato in DM a {user.mention}.', ephemeral=True)
        except discord.Forbidden:
            if interaction.response.is_done():
                await interaction.followup.send('❌ Non posso inviare il DM: l’utente ha i DM chiusi.', ephemeral=True)
            else:
                await interaction.response.send_message('❌ Non posso inviare il DM: l’utente ha i DM chiusi.', ephemeral=True)
        except Exception as e:
            logger.error(f'Errore invio sendtranscript #{ticket_str} a {getattr(user, "id", "?")}: {e}')
            if interaction.response.is_done():
                await interaction.followup.send('❌ Errore durante l’invio del transcript.', ephemeral=True)
            else:
                await interaction.response.send_message('❌ Errore durante l’invio del transcript.', ephemeral=True)
...
class TicketButton(discord.ui.Button):
    ...
    async def callback(self, interaction):
        ...
        try:
            ...
            try:
                await channel.send(f'{interaction.user.mention} 🎫 Ticket creato qui!')
            except Exception:
                pass
...
class CloseTicketView(discord.ui.View):
    ...
    @discord.ui.button(label='Chiudi Ticket', style=discord.ButtonStyle.danger, emoji='🔒', custom_id='ticket_close')
    async def close_ticket(self, interaction: discord.Interaction, button: discord.ui.Button):
        channel = self.cog.bot.get_channel(self.channel_id) if self.channel_id else interaction.channel
        if not channel:
        ...
        ticket_info = self.cog.ticket_owners.get(channel_id, {})
        if isinstance(ticket_info, int):
            ticket_owner = ticket_info
        else:
            ticket_owner = ticket_info.get('owner')
...
class ConfirmCloseView(discord.ui.View):
    ...
    async def confirm_close(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        with open(CLOSED_TICKETS_JSON, 'w', encoding='utf-8') as f:
            json.dump(self.cog.closed_tickets, f, indent=2, ensure_ascii=False)
...
async def setup(bot):
    ...
    try:
        ...
        buttons = persisted_config.get('ticket_buttons', [])
        bot.add_view(TicketView(buttons, persisted_config, cog))
        bot.add_view(CloseTicketView(None, cog))
    except Exception as e:
        logger.error(f'Errore nella registrazione delle persistent views dei ticket: {e}')
